...
class SuffixAutomaton {
  public:
    ...
    SuffixAutomaton(const string &S = "") {
        start = finish = alloc.allocate(S.size() * 2);
        root = new element();
        lastNode = root;
  ...
  private:
    void extend(int number) {
        element *now = new(finish) element(lastNode -> length + 1);
        finish += 1;
        ...
        element *clone = new(finish) element(k -> length + 1, next -> link);
        finish += 1;
