 ...
 * For any other types the HashFunctor template must be specialized
...
template<class DataType, class HashFunctor = std::tr1::hash<DataType> >
class HashSet {
  public:
    ...
    std::pair<iterator, bool> operator+=(const DataType& element) {
    ...
    bool operator<(const DataType& element) const {
...
template<class DataType, class HashFunctor>
const double HashSet<DataType, HashFunctor>::kLoadFactor = 0.8;

using namespace std;

template<class DataType, class HashFunctor>
istream& operator>>(istream& cin, HashSet<DataType, HashFunctor> &object) {
    char operation;
    DataType value;
    while (cin >> operation) {
        cin >> value;
...
int main() {
    ...
    for (HashSet<string>::iterator it = V.begin(); it != V.end(); ++it)
        cout << *it << " ";
